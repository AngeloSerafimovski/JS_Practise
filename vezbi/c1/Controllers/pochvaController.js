const Pochva = require("../Models/Pochva");

exports.createPochva = async (req, res) => {

    try{

        const novaPochva = await Pochva.create(req.body);
...
exports.getPochvaById = async (req, res) => {

    try{

        const pochva = await Pochva.findById(req.params.id);

        if(!pochva){
            return res.status(404).json({
                status:"fail",
                message:"Pochvata ne e pronajdena"
            });
        }
...
exports.updatePochva = async (req, res) => {

    try{

        const pochva = await Pochva.findByIdAndUpdate(
            req.params.id,
            req.body,
            {
                new:true,
...
exports.deletePochva = async (req, res) => {

    try{

        const pochva = await Pochva.findByIdAndDelete(req.params.id);
